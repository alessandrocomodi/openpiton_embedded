...
//`endif
   // Outputs
   // rdtag_w0_y, rdtag_w1_y, rdtag_w2_y, rdtag_y, rdtag_w3_y, 
   rdtag_y,
   so,
   // Inputs
   rclk, se, si, reset_l, sehold, rst_tri_en, index0_x, index1_x,
   index_sel_x, dec_wrway_x, rdreq_x, wrreq_x,

  wrtag_x, wrtag_y,
   // wrtag_w0_y,
   // wrtag_w1_y, wrtag_w2_y, wrtag_w3_y, wrtag_w0_x,
   // wrtag_w1_x, wrtag_w2_x, wrtag_w3_x, 
  ...
  rtap_srams_bist_data
   ...
   input [`L1D_SET_IDX_MASK]    index0_x;     // read/write address0
   input [`L1D_SET_IDX_MASK]    index1_x;     // read/write address1

   input          index_sel_x;  // selects between index1 and index0

   input [`L1D_WAY_COUNT-1:0]    dec_wrway_x;  // way -- functions as a write enable
                                // per 33b
...
l1_tag l1_tag ( .nclk(~clk), .adr(index_y[`L1D_SET_IDX_MASK]), .we(we), .wm(wm),
                                              .din ({wrtag_y,wrtag_y,wrtag_y,wrtag_y}),
                                              .dout(rdtag_y) );
// TRIN_NOT_USED;
`else
   ...
   reg [`L1D_SET_IDX_WIDTH+`L1D_WAY_WIDTH-1:0] write_index_y;
   ...
   always @ (negedge clk)
     begin
	   ...
	   begin
             // if (dec_wrway_y[0])
	            //  ictag_ary[{index_y, 2'b00}] = wrtag_y;
             // if (dec_wrway_y[1])
	            //  ictag_ary[{index_y, 2'b01}] = wrtag_y;
             // if (dec_wrway_y[2])
	            //  ictag_ary[{index_y, 2'b10}] = wrtag_y;
             // if (dec_wrway_y[3])
	            //  ictag_ary[{index_y, 2'b11}] = wrtag_y;
        write_index_y[`L1D_SET_IDX_WIDTH+`L1D_WAY_WIDTH-1 -: `L1D_SET_IDX_WIDTH] = index_y;
        if (dec_wrway_y[0])
   ictag_ary[write_index_y + 0] = wrtag_y;
else if (dec_wrway_y[1])
   ictag_ary[write_index_y + 1] = wrtag_y;
else if (dec_wrway_y[2])
   ictag_ary[write_index_y + 2] = wrtag_y;
else if (dec_wrway_y[3])
   ictag_ary[write_index_y + 3] = wrtag_y;
...
//               case (dec_wrway_y)
//                 4'b0001 : w0[index_y[`L1D_SET_IDX_MASK]] ={30{1'bx}};
//                 4'b0010 : w1[index_y[`L1D_SET_IDX_MASK]] ={30{1'bx}};
//                 4'b0100 : w2[index_y[`L1D_SET_IDX_MASK]] ={30{1'bx}};
//                 4'b1000 : w3[index_y[`L1D_SET_IDX_MASK]] ={30{1'bx}};
//               endcase // case(wrway_y)
//             end
//             else begin
//               case (dec_wrway_y)
//                 4'b0001 : w0[index_y[`L1D_SET_IDX_MASK]] = wrtag_w0_y[29:0];
//                 4'b0010 : w1[index_y[`L1D_SET_IDX_MASK]] = wrtag_w1_y[29:0];
//                 4'b0100 : w2[index_y[`L1D_SET_IDX_MASK]] = wrtag_w2_y[29:0];
//                 4'b1000 : w3[index_y[`L1D_SET_IDX_MASK]] = wrtag_w3_y[29:0];
//               endcase // case(wrway_y)
//             end
//	        end
//     end
...
reg [131:0] cache [127:0];
  ...
  // reg  [32:0]    rdtag_w2_y;
  // reg  [32:0]    rdtag_w3_y;
  reg [`L1D_TAG_ARRAY_REAL_WIDTH-1:0]    rdtag_y;
